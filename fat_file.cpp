...
bool mini_file_delete(FAT_FILESYSTEM *fs, const char *filename)
{
	...
	std::__1::vector<FAT_FILE *> files = fs->files;
	FAT_FILE *file = mini_file_find(fs, filename);
	...
	{
		printf("The file named %s is open now cannot be deleted", filename);
		return false;
	}
